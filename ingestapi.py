...
class IngestApi:
    ...
    def getEntities(self, submissionUrl, entityType):
        ...
        if r.status_code == requests.codes.ok:
            if entityType in json.loads(r.text)["_links"]:
                ...
                for entity in self.getAllObjectsFromSet(json.loads(r.text)["_links"][entityType]["href"], entityType):
                    yield entity

    def getAllObjectsFromSet(self, url, entityType):
        ...
        if r.status_code == requests.codes.ok:
            if "_embedded" in json.loads(r.text):
                ...
                if "next" in json.loads(r.text)["_links"]:
                    for entity2 in self.getAllObjectsFromSet(json.loads(r.text)["_links"]["next"]["href"], entityType):
                        yield entity2
    ...
    def getRelatedEntities(self, relation, entity, entityType):
        ...
        if relation in entity["_links"]:
            ...
            for entity in self.getAllObjectsFromSet(entityUri, entityType):
                yield entity
